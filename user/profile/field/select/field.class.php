 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class profile_field_select extends profile_field_base {
     ...
     * Constructor method.
     *
     * Pulls out the options for the select from the database and sets the the corresponding key for the data if it exists.
     *
     * @param int $fieldid
     * @param int $userid
     */
    public function profile_field_select($fieldid = 0, $userid = 0) {
        // First call parent constructor.
        $this->profile_field_base($fieldid, $userid);

        // Param 1 for select type is the options.
        ...
        foreach ($options as $key => $option) {
            ...
            list($value,$keys)=explode(',',$option);
            
            $this->options[$keys] = format_string($value); // Multilang formatting with filters.
        }        
     ...
     *
     * @param string $value one of the values in select options.
     * @return int options key for the select
     */
