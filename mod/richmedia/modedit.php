...
if (!empty($add)) {
    ...
    course_create_sections_if_missing($course->id, $section);
    $cw = get_fast_modinfo($course->id)->get_section_info($section);
    ...
    if (!empty($fromform->update)) {
        ...
        if (!empty($CFG->enableavailability)) {
            ...
            if ($cm->availableuntil) {
                $cm->availableuntil = strtotime('23:59:59', $cm->availableuntil);
            }
        ...
        if (plugin_supports('mod', $fromform->modulename, FEATURE_MOD_INTRO, true)) {
            $fromform->intro = file_save_draft_area_files($fromform->introeditor['itemid'], $modcontext->id, 'mod_' . $fromform->modulename, 'intro', 0, array('subdirs' => true), $fromform->introeditor['text']);
        ...
        if (!empty($CFG->enableavailability)) {
            ...
            if ($newcm->availableuntil) {
                $newcm->availableuntil = strtotime('23:59:59', $newcm->availableuntil);
            }
        ...
        $sectionid = course_add_cm_to_section($course->id, $fromform->coursemodule, $cw->section);
