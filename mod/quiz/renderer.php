    ...
    */
    public function navigation_panel(quiz_nav_panel_base $panel) {
        ...
        $output .= html_writer::tag('div', $panel->render_end_bits($this),
            array('class' => 'othernavasf quiz-nav__btn'));

        $bcc = $panel->get_button_container_class();
        $output .= html_writer::start_tag('div', array('class' => "quiz-nav__links clearfix $bcc"));



        //$output.=$panel->get_attemptobj()->get_attempt()->currentpage;
        $currentpage=(int)isset($_GET['page'])? $_GET['page']:0;

        if ($currentpage>0){
            $output .= html_writer::start_tag('a', array('class' => 'qnbutton qnbutton--prev', 'href' => new moodle_url('mod/quiz/attempt.php',array('attempt'=>$panel->get_attemptobj()->get_attempt()->id,'page'=>$currentpage-1))));
            $output .= html_writer::tag('i', '',array('class' => 'fa fa-chevron-right'));
            $output .= html_writer::tag('span', 'הקודם');
        ...
        foreach ($panel->get_question_buttons() as $button) {
             $paging=str_replace('שאלה','',$this->render($button));
             $paging=str_replace('עמוד זה','',$paging);
             $arrPages[]=$paging;
        }

        $totalPages=count($arrPages);
        if ($totalPages>$intPaging){
                 $startValue=$currentpage-2;
                 if ($startValue<=0){
                    $startValue=0; 
                 }
                 for($i=$startValue;$i<$currentpage+4;$i++){
                    $output .= $arrPages[$i];
                 }  
        }else{
            foreach ($arrPages as $strValue) {
                $output .= $strValue;
            }
        }

        $mobilePager=($currentpage+1)."/".$totalPages;
        $output .= html_writer::tag('div', $mobilePager, array('class' => 'qnbutton qnbutton--status visible-phone'));

        if ($currentpage<$totalPages-1){
            $output .= html_writer::start_tag('a', array('class' => 'qnbutton qnbutton--next', 'href' =>  new moodle_url('mod/quiz/attempt.php',array('attempt'=>$panel->get_attemptobj()->get_attempt()->id,'page'=>$currentpage+1))));
            $output .= html_writer::tag('i', '', array('class' => 'fa fa-chevron-left'));
            $output .= html_writer::tag('span', 'הבא');
            $output .= html_writer::end_tag('a');
        }
        $output .= html_writer::end_tag('div');
