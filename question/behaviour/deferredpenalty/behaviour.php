...
class qbehaviour_deferredpenalty extends qbehaviour_deferredfeedback {
    ...
    public function process_finish(question_attempt_pending_step $pendingstep) {
        ...
        $userid = $this->qa->get_step(0)->get_user_id();
        $current_user_quiz_attempt = $DB->get_record_sql('SELECT * FROM `mdl_quiz_attempts` WHERE `userid` = '.$userid.' 
        AND `uniqueid` = '.$this->qa->get_usage_id().' ORDER BY `mdl_quiz_attempts`.`attempt` DESC LIMIT 1');
        $quiz = $DB->get_record('quiz', array('id' => $current_user_quiz_attempt->quiz));
        // Attempt 1 => Do not use penalty.
        // Attempt 2 => Use penalty once.
        // Attempt 3 => Use penalty twice.
        //$penalty = 0.1; // Fix penalty of 10% for each quiz attempt. (ignore penalty in question)
        if ($quiz->gradepenalty != 0) {
            $attempt_penalty = ($current_user_quiz_attempt->attempt - 1) * ($quiz->gradepenalty/100);
            $adjustment = 1.0 - $attempt_penalty;
        } else {
            // $quiz->gradepenalty == 0   => No penalty.
            $attempt_penalty = 0;
            $adjustment = 1;
        }

        // Make sure we are not getting negative grades.
