...
class login_signup_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('passwordunmask', 'password', get_string('password'), 'maxlength="20" minlength="6" size="12" placeholder="'.get_string('passwordplaceholder').'"');
        $mform->setType('password', PARAM_RAW);
        $mform->addRule('password', get_string('missingpassword'), 'required', null, 'client');

        //$mform->addElement('text', 'tesssst', 'test_string');
...
*/
        ...
        if (!empty($CFG->sitepolicy)) {
            ...
            $mform->addElement('checkbox', 'policyagreed', get_string('policyagreementclick', 'moodle',$CFG->sitepolicy));
            $mform->addRule('policyagreed', get_string('policyagree'), 'required', null, 'client');
        }
       
       
        //TO DO order this list via function
        
        $arrResult=$mform->_elements;
        $arrTemp =$mform->_elements;
        
        $arrResult[3]=$arrTemp[5];
        $arrResult[5]=$arrTemp[3];
        $arrResult[4]=$arrTemp[6];
        $arrResult[6]=$arrTemp[4];
        
        $mform->_elements=$arrResult;
    ...
    function definition_after_data(){
        ...
        $mform->applyFilter('username', 'strtolower');
    }
    ...
    function validation($data, $files) {
        ...
        if ((empty($data['profile_field_birthdaydate']) || $data['profile_field_birthdaydate'] == 'שנת לידה')
            ) {
            $errors['profile_field_birthdaydate'] = 'נא לבחור שנת לידה';
        } 
